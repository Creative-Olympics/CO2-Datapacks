...
# Usage: execute as [...] at [reloc] run function #lib:relocalize_player
# Note: the relocalization will take 3 seconds
# tellraw @a [{"selector":"@s"},{"text":" > lib:relocalize_player/interface","color":"gray"}]
...
execute at @s summon block_display run function lib:relocalize_player/setup_camera
