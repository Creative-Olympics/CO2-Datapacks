...
execute rotated ~ 0 positioned ^ ^ ^0.7 unless predicate prophunt:can_climb run function prophunt:features/climb/stop

# # Stop climb if there is a block above the player
execute rotated ~ 0 positioned ^ ^ ^-0.2 positioned ~ ~2 ~ unless block ~ ~ ~ #prophunt:cant_climb_on run function prophunt:features/climb/stop
...
execute if entity @s[nbt={OnGround:1b}] rotated ~ 0 positioned ^ ^ ^-0.2 positioned ~ ~-0.001 ~ unless block ~ ~ ~ #prophunt:cant_climb_on run function prophunt:features/climb/stop

# Stop climb if the player climbed for too long
execute if score @s prophunt.climb matches 20.. run function prophunt:features/climb/stop
