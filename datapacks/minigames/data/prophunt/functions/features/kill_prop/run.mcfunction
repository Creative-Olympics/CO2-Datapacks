...
# tellraw @a [{"selector":"@s"},{"text":" > prophunt:features/kill_prop/run","color":"gray"}]

function prophunt:utils/get_associated_entities

tellraw @a[team=prophunt] [{"text":"Prophunt > ","color":"gray"},{"selector":"@p[tag=prophunt.current.hunter]","color":"red"},{"text":" ⚔ ","color":"gray"},{"selector":"@p[tag=prophunt.current.player]","color":"blue"}]

# Remaining props title
scoreboard players set #prophunt.prop_count data -1
...
# title @a[tag=prophunt.current.player] title {"text":"Discovered 😢","color":"red"}
...
playsound minecraft:block.anvil.place master @a[team=prophunt] ~ ~ ~ 2 2 1
...
execute as @p[tag=prophunt.current.player,tag=prophunt.is_locked] at @s run function prophunt:features/lock_position/unlock
