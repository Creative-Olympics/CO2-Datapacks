...
execute as @e[tag=wallstreet.villager_marker.new] run function lib:id/get_id

$execute at @e[tag=wallstreet.villager_marker.new] run summon minecraft:villager ~ ~ ~ {CustomName:'{"text":"Villager $(name)","color":"yellow","bold":true,"italic":true}',CustomNameVisible:1b,VillagerData:{profession:"$(profession)",type:"$(type)"},Tags:["wallstreet.entity","wallstreet.villager","wallstreet.villager.new"]}
