...
function common:id/get_child
kill @e[tag=wallstreet.villager.gui,distance=..3.5]
kill @e[tag=wallstreet.villager.protection,distance=..3.5]
data modify entity @e[tag=common.id.child,limit=1,sort=nearest] CustomNameVisible set value 1b
tag @s remove wallstreet.villager_marker.talking
tag @e[tag=common.id.child] remove wallstreet.villager.talking
