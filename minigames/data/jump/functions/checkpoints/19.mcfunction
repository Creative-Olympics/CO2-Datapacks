...
execute as @a[team=jump,scores={jump.step=18}] unless data entity @s Inventory[{Slot:4b}] run item replace entity @s hotbar.4 with compass{display:{Name:'{"text":"Next checkpoint","color":"dark_purple","bold":true,"italic":true}'},LodestoneDimension:"minecraft:overworld",LodestoneTracked:0b,LodestonePos:{X:-336,Y:111,Z:1}} 1
...
execute at @e[tag=jump.checkpoint.19] as @a[distance=..2,scores={jump.step=18}] at @s run clear @s
execute at @e[tag=jump.checkpoint.19] as @a[distance=..2,scores={jump.step=18}] at @s run function #global:animated_title {type:"finish"}
execute at @e[tag=jump.checkpoint.19] as @a[distance=..2,scores={jump.step=18}] at @s run stopsound @s music
...
execute at @e[tag=jump.checkpoint.19] as @a[distance=..2,scores={jump.step=18}] at @s run team leave @s