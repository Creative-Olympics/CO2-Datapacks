...
execute as @a[team=jump,scores={jump.step=1}] unless data entity @s Inventory[{Slot:4b}] run item replace entity @s hotbar.4 with compass{display:{Name:'{"text":"Next checkpoint","color":"dark_purple","bold":true,"italic":true}'},LodestoneDimension:"minecraft:overworld",LodestoneTracked:0b,LodestonePos:{X:-293,Y:105,Z:21}} 1
...
execute at @e[tag=jump.checkpoint.2] as @a[distance=..2.5,scores={jump.step=1}] at @s run clear @s
