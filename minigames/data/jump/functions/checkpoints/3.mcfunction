...
execute as @a[team=jump,scores={jump.step=2}] unless data entity @s Inventory[{Slot:4b}] run item replace entity @s hotbar.4 with compass{display:{Name:'{"text":"Next checkpoint","color":"dark_purple","bold":true,"italic":true}'},LodestoneDimension:"minecraft:overworld",LodestoneTracked:0b,LodestonePos:{X:-277,Y:103,Z:20}} 1
...
execute at @e[tag=jump.checkpoint.3] as @a[distance=..1.5,scores={jump.step=2}] at @s run clear @s
