...
execute as @a[team=jump,scores={jump.step=7}] unless data entity @s Inventory[{Slot:4b}] run item replace entity @s hotbar.4 with compass{display:{Name:'{"text":"Next checkpoint","color":"dark_purple","bold":true,"italic":true}'},LodestoneDimension:"minecraft:overworld",LodestoneTracked:0b,LodestonePos:{X:-271,Y:110,Z:9}} 1
...
execute at @e[tag=jump.checkpoint.8] as @a[distance=..2,scores={jump.step=7}] at @s run clear @s
...
execute at @e[tag=jump.checkpoint.8] as @a[distance=..2,scores={jump.step=7}] at @s run scoreboard players add @s jump.time_left 100
execute at @e[tag=jump.checkpoint.8] as @a[distance=..2,scores={jump.step=7}] at @s run title @s title {"text": " "}
execute at @e[tag=jump.checkpoint.8] as @a[distance=..2,scores={jump.step=7}] at @s run title @s subtitle {"text": "+5 seconds","color":"gray","bold":true,"italic":true}
