...
execute as @a[team=jump,scores={jump.step=12}] unless data entity @s Inventory[{Slot:4b}] run item replace entity @s hotbar.4 with compass{display:{Name:'{"text":"Next checkpoint","color":"dark_purple","bold":true,"italic":true}'},LodestoneDimension:"minecraft:overworld",LodestoneTracked:0b,LodestonePos:{X:-340,Y:106,Z:29}} 1
...
execute at @e[tag=jump.checkpoint.13] as @a[distance=..2,scores={jump.step=12}] at @s run clear @s
