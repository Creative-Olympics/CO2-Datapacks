...
execute as @a[team=jump,scores={jump.step=15}] unless data entity @s Inventory[{Slot:4b}] run item replace entity @s hotbar.4 with compass{display:{Name:'{"text":"Next checkpoint","color":"dark_purple","bold":true,"italic":true}'},LodestoneDimension:"minecraft:overworld",LodestoneTracked:0b,LodestonePos:{X:-327,Y:103,Z:12}} 1
...
execute at @e[tag=jump.checkpoint.16] as @a[distance=..2,scores={jump.step=15}] at @s run clear @s
