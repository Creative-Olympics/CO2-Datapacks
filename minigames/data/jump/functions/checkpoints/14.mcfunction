...
execute as @a[team=jump,scores={jump.step=13}] unless data entity @s Inventory[{Slot:4b}] run item replace entity @s hotbar.4 with compass{display:{Name:'{"text":"Next checkpoint","color":"dark_purple","bold":true,"italic":true}'},LodestoneDimension:"minecraft:overworld",LodestoneTracked:0b,LodestonePos:{X:-357,Y:106,Z:25}} 1
...
execute at @e[tag=jump.checkpoint.14] as @a[distance=..2,scores={jump.step=13}] at @s run clear @s
