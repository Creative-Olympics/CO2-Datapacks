...
execute as @a[team=jump,scores={jump.step=14}] unless data entity @s Inventory[{Slot:4b}] run item replace entity @s hotbar.4 with compass{display:{Name:'{"text":"Next checkpoint","color":"dark_purple","bold":true,"italic":true}'},LodestoneDimension:"minecraft:overworld",LodestoneTracked:0b,LodestonePos:{X:-346,Y:102,Z:16}} 1
...
execute at @e[tag=jump.checkpoint.15] as @a[distance=..2,scores={jump.step=14}] at @s run clear @s
